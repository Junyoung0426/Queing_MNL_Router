...
class QueueEnv:
    ...
    def _pred_u_list_from_theta(self, x_ctx: np.ndarray, S: List[int]) -> List[float]:
        # disjoint router: u_hat[k] = z(x) · theta[S[k]]
        with torch.no_grad():
            u_hat_t = self.router.scores_for_S_from_ctx(x_ctx, S)  # (K,)
        if isinstance(u_hat_t, torch.Tensor):
            return [float(v) for v in u_hat_t.detach().cpu().tolist()]
        return [float(v) for v in np.asarray(u_hat_t, dtype=np.float32).tolist()]
    ...
    def _select_best_by_exhaustive_batch(self, noise_vectors: torch.Tensor) -> Tuple[int, int, List[int]]:
        """
        disjoint router + TS:
          theta_opt[j] = theta[j] + noise[j]
          u_samp(b, model, m) = z(b) · theta_opt[model, :, m]
          for each slate S:
            u_samp(b, k, m) = u_samp(b, S[k], m)
            u_optim(b, k) = max_m u_samp(b, k, m)
            dep_optim(b, S) = sigmoid(logsumexp_k u_optim(b, k))
        """
        q = self.queue_router
        ...
        # noise_vectors: (N,d,M)
        with torch.no_grad():
            theta_opt = self.router.theta.unsqueeze(-1) + noise_vectors.to(self.device, dtype=torch.float32)  # (N,d,M)

        bs = int(self.exploit_job_batch)
        for st in range(0, int(q_pos.numel()), bs):
            ed = min(st + bs, int(q_pos.numel()))
            ...
            with torch.no_grad():
                # (B,d)
                z_ctx = self.router.B_projection(Xb).to(dtype=torch.float32)

                # scores_all: (B, n_models, M)
                scores_all = torch.einsum("bd,ndm->bnm", z_ctx, theta_opt)

                # model-level optimism first: (B, n_models)
                u_optim_all = scores_all.max(dim=2).values

                # then gather to slates: (B, C, K)
                u_optim = u_optim_all[:, self.S_tensor]

                # MNL departure: (B, C)
                vals = torch.sigmoid(torch.logsumexp(u_optim, dim=2))
    ...
    def step(self):
        ...
        if len(self.queue_router) > 0:
            ...
            if x_ctx is None:
                # 사용자가 원한 스케줄: t_eff = max_steps 고정
                t_eff = int(self.config.max_steps)

                term1 = self.d * math.log(1.0 + (t_eff * self.K) / (self.d * self.lambda_0))
                term2 = 4.0 * math.log(max(2.0, float(t_eff)))  # log(1)은 0이라 완화
                term3 = self.kappa * math.sqrt(self.lambda_0)
                alpha_t = (0.5 * self.kappa * math.sqrt(term1 + term2) + term3) * self.config.alpha_coef

                noise_vectors = self.router.sample_theta_noise(alpha_t=alpha_t, M=self.M_sample)  # (N,d,M)
