# B_MNL_Bandit/base_line/3qucb/queue_env.py
...
def _bandit_root_from_here() -> str:
    # Path: .../B_MNL_Bandit/base_line/3qucb/queue_env.py
...
_base_qenv_mod = _load_module_from_path(
    "bandit_queue_env_for_qucb",
    os.path.join(_BANDIT, "queue_env.py"),
)
BaseQueueEnv = _base_qenv_mod.QueueEnv


class QueueEnvQUCB(BaseQueueEnv):
    """
    Baseline (3): Q-UCB
      - Implements Algorithm 1 style from Krishnasamy et al. (2021).
      - K=1 assumption (Single arm selection).
      - Queue Discipline: FIFO (Always selects the head of the queue).
      - Exploration Schedule: Bern( min(1, 3 * N * (log t)^2 / t ) ), where N = number of arms.
      - Exploration Action: Selects S via round-robin or random.
      - Exploitation Action: Selects S via UCB index:
          idx_S(t) = mu_hat_S(t-1) + sqrt( (log t)^2 / (2 * T_S(t-1)) )
      - Updates: Tracks T_S (counts) and sum_y (rewards).
    ...
    def _pick_S_ucb(self, t: int) -> int:
        """
        Exploitation: Select arm with highest UCB index.
        Unpulled arms are treated as having +inf index.
        """
        
        T = self.T_S
        nz = T > 0

        mu = np.zeros(self.C_size, dtype=np.float64)
        mu[nz] = self.sum_y[nz] / T[nz].astype(np.float64)

        idx = np.empty(self.C_size, dtype=np.float64)
        idx[:] = -1e30
        idx[~nz] = np.inf

        log_t_sq = self._log_squared_safe(t)
        bonus = np.sqrt(log_t_sq / (2.0 * T[nz].astype(np.float64)))
        idx[nz] = mu[nz] + bonus
    ...
    def step(self):
        ...
        if len(self.queue_router) > 0:
            ...
            y = 1.0 if departed else 0.0
            self.T_S[int(c_idx)] += 1
            self.sum_y[int(c_idx)] += float(y)
        ...
        # State update for compatibility
        self.A_prev = A_curr
        self.E_prev = 0
        ...
        if (self.steps % self.config.log_every == 0):
            ...
            print(f"[QUCB step={self.steps}] regret(avg)={avg_reg:.6f} Q-gap={q_gap:.3f} Q_r={Q_r} Q_o={Q_o}")
