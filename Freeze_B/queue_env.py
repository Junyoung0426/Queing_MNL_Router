...
class QueueEnv:
    ...
    - router: 학습 중인 MNLRouter (외부에서 전달 가능; B는 freeze된 상태로 가정)
    - queue_router: 에이전트 큐
    ...
    def __init__(
        ...
        acc_mat: np.ndarray,   # (N_jobs, K)
        util_mat: np.ndarray,  # (N_jobs, K)
        config: QueueConfig,
        router: Optional[MNLRouter] = None,
    ):
        self.X_ctx = X_ctx
        ...
        if router is None:
            ...
            assert self.router.d_proj == config.d_proj, "router.d_proj != config.d_proj"
    ...
    def step(self):
        ...
        # standard regret 계산을 위한 snapshot
        ...
        if len(self.queue_router) > 0:
            ...
            # SupCon buffer: 실제 선택된 서버만 저장 (online에선 lambda_sc=0으로 보통 끔)
            ...
            # joint update: θ + B(보통 freeze) + V_inv
